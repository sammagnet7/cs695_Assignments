...
#define FRAC_A 7  // Fraction representing the number of iterations VM1 runs
#define FRAC_B 3  // Fraction representing the number of iterations VM2 runs
...
/*
    ...
    Description:
    This function runs two virtual machines (VMs) alternately based on a predefined ratio. It uses the KVM API to execute the VMs.
    The function selects which VM to run based on a counter that tracks the number of loop iterations.
...
void kvm_run_vm(struct vm *vm1, struct vm *vm2)
{
    ...
    int total_iteration = FRAC_A + FRAC_B;      // Calculate the total number of iterations based on the fractions
    ...
    while (1)
    {   
        // Determine which VM to run based on the counter
        if (vm_select % total_iteration < FRAC_A) 
            vm = vm1;   // VM1 runs 7 times out of every 10 iterations
        else 
            vm = vm2;   // VM2 runs 3 times out of every 10 iterations
        ...
        if (kvm_ret < 0 && vm->vcpus->kvm_run->exit_reason != KVM_EXIT_INTR)
        {
         ...
         // Increment the iteration counter and reset if needed
        vm_select++;
        if (vm_select == total_iteration)
            vm_select = 0; // Reset the counter after 10 iterations
    }
