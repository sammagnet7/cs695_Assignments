...
int main()
{
    ...
    char ns_pid_file_name[50];

    sprintf(ns_pid_file_name,"/proc/%d/ns/pid", child_pid);
    
    int fd1 = open(ns_pid_file_name, O_RDONLY); /* Get descriptor for namespace */
    
    if (fd1 == -1)
        errExit("open");

    if (setns(fd1, CLONE_NEWPID) == -1) /* Join that namespace */
        errExit("setns");
    ...
    if (fork() == 0)
    {
        ...
        char ns_uts_file_name[50];
        
        sprintf(ns_uts_file_name,"/proc/%d/ns/uts", child_pid);
        
        int fd2 = open(ns_uts_file_name, O_RDONLY); /* Get descriptor for namespace */
        
        if (fd2 == -1)
        errExit("open");

        if (setns(fd2, CLONE_NEWUTS) == -1) /* Join that namespace */
            errExit("setns");
