...
run()
{
    ...
    chown -R root:root "$CONTAINERDIR/$NAME/rootfs"
    chmod -R 755 "$CONTAINERDIR/$NAME/rootfs"
    
    unshare --uts --pid --net --mount --ipc --fork --kill-child \
            --mount-proc="$CONTAINERDIR/$NAME/rootfs/proc" \
                    chroot "$CONTAINERDIR/$NAME/rootfs" /bin/bash -c \
                            "mount -t sysfs sysfs /sys && $INIT_CMD_ARGS"
...
exec()
{
    ...
    # nsenter --target $CONTAINER_INIT_PID --uts --pid --net --mount --ipc \
    #     unshare --fork --kill-child \
    #         chroot "$PWD/$CONTAINERDIR/$NAME/rootfs" /bin/bash -c \
    #                "$EXEC_CMD_ARGS"

    nsenter --target $CONTAINER_INIT_PID \
        --uts --pid --net --mount --ipc --root --wd \
        /bin/bash -c "$EXEC_CMD_ARGS"
